...
| **Music** | `/play` accepts a song name, SoundCloud link, or a Spotify/Apple Music track/album/playlist link. Runs on a **self-hosted** Lavalink node (see `/lavalink-server`, deployed as its own Render service — see its README for setup) instead of free public nodes, which is what was causing the intermittent 403s/failures before: free nodes are shared, rotate passwords, and go down with no warning. That node runs the LavaSrc plugin, which resolves Spotify/Apple Music links natively server-side (needs `SPOTIFY_CLIENT_ID`/`SPOTIFY_CLIENT_SECRET` set on the *lavalink-server* service, not the bot — see `lavalink-server/README.md`) and searches SoundCloud (`MUSIC_SEARCH_PLATFORM`, see `.env.example`) for the actual audio, since Spotify/Apple Music can't be streamed outside their own apps and YouTube search gets blocked from cloud hosting IPs like Render's. A second node can be added for failover (see commented-out block in `index.js`) — a search that fails on one automatically retries on any other connected node. Requests only work while the requester is in the Music Room (configurable in `config/serverConfig.js` under `music`). |
